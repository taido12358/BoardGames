...
> Quy tắc deployment: CI/CD, environment management, release process.
...
- Secret production không nằm trong repo, không nằm trong image — inject lúc deploy (xem `rule-security.md`).
...
- PR đỏ CI thì không merge (xem `rule-git.md`).
...
Dự án **không dùng migration** — schema bootstrap bằng raw SQL idempotent trong `Program.cs` chạy lúc app khởi động (xem `rule-database.md`). Hệ quả cho deploy:
