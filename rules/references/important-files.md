...
- `frontend/` (mới 2026-09-11) — Vitest, 26 test, `npm run test`. `src/games/vaybat/types.test.ts` + `src/games/oanquan/types.test.ts` (helper hiển thị/gợi ý nước đi thuần của từng game). `src/platform/gameStore.test.ts` (merge `isMine` trong `upsertRoom`, giới hạn `chatMessages`, `rematchInvite`, `setPlayerName`). `src/games/oanquan/OAnQuanBoard.test.tsx` (8 test, React Testing Library — ví dụ mẫu test component đầu tiên của repo). Config ở `vite.config.ts` mục `test` (`environment: "jsdom"`, `setupFiles: ["./src/test-setup.ts"]` — polyfill `localStorage` thủ công + `afterEach(cleanup)` thủ công vì dự án không bật `globals: true`). `BangBoard.tsx`/`VayBatBoard.tsx`/`ChatPanel.tsx`/`AdminPage.tsx` chưa có test component — xem [`../coding/testing.md`](../coding/testing.md).
