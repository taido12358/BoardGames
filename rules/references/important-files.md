...
- `frontend/` (mới 2026-09-11) — Vitest, 54 test, `npm run test`. `src/games/vaybat/types.test.ts` + `src/games/oanquan/types.test.ts` (helper hiển thị/gợi ý nước đi thuần của từng game). `src/platform/gameStore.test.ts` (merge `isMine` trong `upsertRoom`, giới hạn `chatMessages`, `rematchInvite`, `setPlayerName`). `src/games/oanquan/OAnQuanBoard.test.tsx` (8 test, React Testing Library — ví dụ mẫu test component đầu tiên của repo) + `src/platform/ChatPanel.test.tsx` (7 test) + `src/components/AdminPage.test.tsx` (6 test — mock `global.fetch` theo URL) + `src/games/bang/BangBoard.test.tsx` (8 test — luồng bỏ bài khi vượt giới hạn tay bài) + `src/games/vaybat/VayBatBoard.test.tsx` (7 test — mock hình học SVG + workaround jsdom thiếu `PointerEvent`). Config ở `vite.config.ts` mục `test` (`environment: "jsdom"`, `setupFiles: ["./src/test-setup.ts"]` — polyfill `localStorage` + `Element.prototype.scrollTo`/`scrollIntoView` (jsdom không triển khai) + `afterEach(cleanup)` thủ công vì dự án không bật `globals: true`). Không còn component board nào thiếu test — xem [`../coding/testing.md`](../coding/testing.md).
