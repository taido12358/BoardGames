...
- **Gate tính năng dev-only KHÔNG được chỉ dựa vào `IHostEnvironment.IsDevelopment()`** — vì lý do ở dòng trên, `docker-compose.yml` (máy dev local) chạy `ASPNETCORE_ENVIRONMENT=Production`, nên `IsDevelopment()` luôn `false` ở đúng nơi cần bật tính năng. Pattern đúng (đã dùng 2 lần: `SmtpOtpSender.DevLogOtpEnabled`, `BangDebugController.IsEnabled`): `env.IsDevelopment() || config.GetValue<bool>("Namespace:CoTuTao")`, với compose set cờ đó mặc định `true` còn deploy thật không set gì (mặc định `false`, an toàn).
